...
class VideoDetector:
    ...
    # -- Form predictions --
    def inference(self, img, detection_threshold=0.35): # Threshold of 35% because false negatives are better that false positives
        with torch.no_grad():
                ...
                for idx, points in enumerate(self.parking_boxes):
                    ...
                    if occupied:
                        ...
                        if self.slot_status[idx]['occupied']:
                            ...
                            if duration / 60 < 1:
                                log = f"[{time.strftime('%H:%M:%S')}] Slot {idx+1} VACATED - Occupied for {duration:.1f} seconds"
                            elif duration / 60 >= 1 and duration / 3600 < 1:
                                duration = duration / 60
                                log = f"[{time.strftime('%H:%M:%S')}] Slot {idx+1} VACATED - Occupied for {duration:.1f} minutes"
                            elif duration / 3600 >= 1:
                                duration = duration / 3600
                                log = f"[{time.strftime('%H:%M:%S')}] Slot {idx+1} VACATED - Occupied for {duration:.1f} hours"
                            self.log_messages.append(log)
                            self.save_logs_to_db(log)
